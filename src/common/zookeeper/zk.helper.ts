...
export class ZkHelper implements zookeeperHelper{
    ...
    private getUseMethodNames(){
        return Object.getOwnPropertyNames(ZkHelper.prototype)
            .filter(key => !(["constructor","helper","createInstance","hasConnect","connect"].find(name => key == name)) && typeof ZkHelper.prototype[key] === "function")
    }
    /**
     * 是否还在连接中
     */
     ...
     */
    public getState(): any{
        if(!this.client){ this.createServer() }
     ...
     * @param mode 
     */
    public mkdirp(path: string,data: string = "",acls: any[] = ACLS.OPEN_ACL_UNSAFE,mode: number = CreateMode.PERSISTENT): Promise<unknown>{
        let resolve:Function,reject:Function
     ...
     * @param mode 
     */
    public create(path: string,data: string = "",acls: any[] = ACLS.OPEN_ACL_UNSAFE,mode: number = CreateMode.PERSISTENT): Promise<unknown>{
        let resolve:Function,reject:Function
     ...
     */
    private listener(){
        if(!this.client){ throw new Error("请初始化zookeeper client.") }
        this.client.on('connected',() => { 
            this.logger.log(`[zookeeper-listener] zookeeper is connected. sessionID:${this.client.getSessionId().toString('hex')}`) 
        })
        ...
        this.client.on('connectedReadOnly',() => {
            this.logger.log(`[zookeeper-listener] zookeeper is connectedReadOnly. sessionID:${this.client.getSessionId().toString('hex')}`) 
        })
    ...
    public connect(): Promise<unknown>{
        ...
        if(!this.client){ this.createServer() }
        this.client.once('connected',() => { resolve != empty && resolve(),reject = resolve = empty,timeHandler && clearTimeout(timeHandler) })
        this.client.connect()
        //连接30秒超时
        timeHandler = setTimeout(() => { reject && reject(),reject = resolve = empty },30 * 1000)
        return new Promise((res,rej) => { resolve = res,reject = rej })
    }
