...
export class ConsistentHash extends BalanceBase implements balancers{
    ...
    private getHash(digest:string,idx:number):number{
        const length = 4
        if(!digest || digest.length < 3 + idx * length){ throw new Error("[ConsistentHash-getHash] 参数错误-digest") }
        //计算
        const idxNumbers:number[] = Array.from({ length },(empty,num) => Number(digest[num + idx * 4]) & 0xff << num * 8)
        //返回hash
    ...
    private lookForKey(hash:number):addressSender{
        ...
        this.oderKeys[length] < hash && Object.keys(Array.from({ length })).reverse().find(currentNum => {
