...
export class BalanceBase implements balancers{
     ...
     * @param defaultWeight 
     */
    protected parseAddressList(addressList: addressListType,defaultWeight:number = this.defaultWeight):addressWeightMap{
        if(!Array.isArray(addressList) || addressList.length <= 0){ throw new Error("[balanceBase-parseAddressList] 参数错误-addressList") }
    ...
    updateAddressList(addressList: addressListType,defaultWeight:number = this.defaultWeight): addressWeightMap {
        if(!Array.isArray(addressList)){ return new Map() }
