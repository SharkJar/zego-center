    ...
    if (delete target.prototype[propertyName]) {
      Object.defineProperty(target.prototype, propertyName, {
        get: function () {
          //初始化 获取值
          instance =
            instance || typeof data === 'undefined'
...
export class Factory {
  ...
  static async useClass<TOutput = any>(params: useClassParams) {
    ...
    if (!IsInjectable(creator.prototype.constructor)) {
  ...
  static async create<TOutput = any>(injectors: any[], creator: constructor | Function, dirName?: any) {
    ...
    if (Factory.craetorInstanceMap.has(dirName)) {
      return Factory.craetorInstanceMap.get(dirName);
    }
    ...
    if (creator.prototype && creator.prototype.constructor == creator) {
      ...
      instance = await useFactory(
        ...(await Promise.all(injectors.map((type: any) => Factory.createConstructor(type)))),
      );
    }
    //把他放入实例队列中
  ...
  static async createConstructor<TOutput = any>(creator: constructor | any) {
    ...
    if (Factory.craetorInstanceMap.has(creator)) {
      return Factory.craetorInstanceMap.get(creator);
    }
    const constructor = creator.prototype.constructor;
    if (!IsInjectable(creator.prototype.constructor)) {
      throw new Error(`请在${creator.name}定义的时候加上 @Injectable()`);
