...
export class CenterService{
    ...
    private async nextTick(){
        ...
        if(tasks.length >= 0){
            ...
            //断开zk 当前注册节点会跟着一起删除
            if(isNeedBreak){  return await this.helper.close()  }
            ...
            while(currentTask = tasks.shift()){
                try{
                    ...
                    //写入节点数据
                    await this.helper.setData(String(path),JSON.stringify({ ip:currentTask.serverIP,port:currentTask.serverPort,weight }))
                }catch(error){}
            }
        }
