...
var ZkHelper = (function () {
    ...
    ZkHelper.prototype.startTask = function () {
        var _a;
        while (this.tasks.length) {
            ...
            var method = task.method, args = task.args, success = task.success, error = task.error;
            try {
                var result = (_a = this.client)[method].apply(_a, args);
    ...
    ZkHelper.prototype.startTaskError = function (err) {
        while (this.tasks.length) {
            var task = this.tasks.shift();
            if (!task) {
                continue;
    ...
    ZkHelper.prototype.callLib = function (methodName) {
        ...
        var _b = this.createPromise(), timeout = _b.timeout, success = _b.success, error = _b.error, promise = _b.promise;
        var isConnected = this.hasConnect();
        if (isConnected) {
            var result = (_a = this.client)[methodName].apply(_a, args);
            success(result);
        }
        else {
            this.tasks.push({ method: methodName, success: success, error: error, args: args });
            this.connect();
        }
        return promise;
    };
    ...
    ZkHelper.prototype.listener = function () {
        ...
        process.removeAllListeners('uncaughtException').on('uncaughtException', function (err) {
            _this.logger.log('Error caught in uncaughtException event:', JSON.stringify(err));
        });
    };
